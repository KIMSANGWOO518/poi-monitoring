...
// POI 데이터 타입 정의 (status 제거 + Store_region 추가)
interface POIData {
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <span className="text-sm">
          {label}:{" "}
          {selected.length === 0 ? "선택 안함" : selected.length === options.length ? "전체" : `${selected.length}개 선택`}
        </span>
      ...
      {isOpen && (
        <div
          className="absolute z-[9999] mt-2 w-64 bg-white border border-gray-200 rounded-lg shadow-lg"
          style={{ zIndex: 9999 }}
        >
          <div className="p-2 border-b border-gray-200 flex gap-2">
            <button onClick={selectAll} className="flex-1 px-2 py-1 text-xs bg-blue-50 hover:bg-blue-100 text-blue-700 rounded transition">
              전체 선택
            </button>
            <button onClick={clearAll} className="flex-1 px-2 py-1 text-xs bg-gray-50 hover:bg-gray-100 text-gray-700 rounded transition">
              전체 해제
            </button>
          </div>
          <div className="max-h-60 overflow-y-auto">
            {options.map((option) => (
              <label key={option} className="flex items-center px-3 py-2 hover:bg-gray-50 cursor-pointer transition">
                <input type="checkbox" checked={selected.includes(option)} onChange={() => toggleOption(option)} className="mr-3" />
                <span className="flex items-center gap-2 text-sm">
                  {colors[option] && <span className="w-3 h-3 rounded-full" style={{ backgroundColor: colors[option] }} />}
                  {option}
                </span>
              </label>
            ))}
...
function LoginForm({ onLogin }: { onLogin: (username: string) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (username === "test_2025" && password === "1234") onLogin(username);
      else if (username === "poi_2025" && password === "7777") onLogin(username);
      else if (username === "dynamic_2025" && password === "8888") onLogin(username);
      else if (username === "gis_2025" && password === "4321") onLogin(username);
      else if (username === "park_2025" && password === "4321") onLogin(username);
      else if (username === "recruit_2025" && password === "99998888") onLogin(username);
...
function MapContent({ currentUser, onLogout }: { currentUser: string; onLogout: () => void }) {
  ...
  const getMarkerColor = (franchiseName: string): string => franchiseColors[franchiseName] || "#0000FF";
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(fetchPOIData, 100);
    return () => clearTimeout(timer);
  }, []);

  // 필터링된 데이터 (프랜차이즈만)
  const filteredPoiData = poiData.filter((poi) => selectedFranchises.length === 0 || selectedFranchises.includes(poi.Franchise_name));

  // 지도 중심 좌표 계산
  const getMapCenter = (): [number, number] => {
    if (filteredPoiData.length > 0) {
      return [parseFloat(filteredPoiData[0].Store_lat), parseFloat(filteredPoiData[0].Store_long)];
    }
      ...
      <main className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center">
            ...
            />
            <h1 className="text-3xl font-bold text-gray-800">공간플랫폼개발그룹 POI 모니터링 지도</h1>
          </div>
        ...
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-gray-700">프랜차이즈 매장 위치 모니터링</h2>
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-4 text-sm">
              <span className="font-medium">프랜차이즈:</span>
              {franchiseOptions.map((franchise) => (
                <div key={franchise} className="flex items-center gap-1">
                  <span className="w-3 h-3 rounded-full" style={{ backgroundColor: getMarkerColor(franchise) }} />
                  <span>{franchise}</span>
                </div>
              ))}
            </div>
        ...
        <div className="mb-8">
          <div className="w-full h-[700px] border border-gray-300 rounded-lg overflow-hidden shadow-lg">
            <MapContainer center={getMapCenter()} zoom={11} style={{ height: "100%", width: "100%" }}>
              <TileLayer
              ...
              {filteredPoiData.map((poi, index) => {
                ...
                return (
                  <CircleMarker
                    ...
                    color="#ffffff"
                    weight={2}
                    fillOpacity={0.75} // ✅ 고정값 (status 제거했으니)
                  >
                    <Popup maxWidth={350}>
                      <div className="p-3">
                          ...
                          {poi.Store_region && (
                            <div className="flex items-start">
                              ...
                              <div>
                                ...
                                <span className="ml-1 text-gray-600">{poi.Store_region}</span>
                              </div>
                            </div>
                          )}

                          <div className="mt-2 pt-2 border-t border-gray-100">
                            <p className="text-xs text-gray-500">좌표: {poi.Store_lat}, {poi.Store_long}</p>
                          </div>
                        </div>
                      </div>
                    </Popup>
        ...
        <div className="text-center">
          <p className="text-gray-600">매장 정보는 매주 수요일에 업데이트되며, 마커를 클릭하면 상세 정보를 확인할 수 있습니다.</p>
        </div>
      </main>
    </div>
  );
